...
| **02_agente_ia.gs** | Puente hacia **Groq** (la IA gratuita que usa el agente). Es el único lugar donde vive la API key: se guarda en las *Propiedades del Script* ejecutando `guardarClaveGroq()` una sola vez, así el frontend publicado en GitHub Pages nunca la expone. `01_backend_principal.gs` lo llama desde `doPost` con la acción `agente_consulta`. |
