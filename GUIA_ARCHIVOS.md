...
| **auth.js** | Login corporativo con **contraseña personal por usuario** y recuperación por correo. La contraseña nunca sale del dispositivo: se deriva un verificador con PBKDF2 (150.000 vueltas) y lo valida `03_auth.gs` contra la hoja `Usuarios`. Guarda una copia local del verificador para poder **entrar sin señal** en un dispositivo ya usado, y mantiene la contraseña genérica solo para quien todavía no definió la suya. Sesión en `localStorage` por 24 horas. También administra los usuarios "extra" agregados desde la app. Detalle en [DOCUMENTACION_CONTRASENAS.md](DOCUMENTACION_CONTRASENAS.md). |
...
| **historial-local.js** | Guarda en el celular una copia del historial de Carga, Calidad y Ticketera para poder verlo **sin señal** (Hallazgo 13). La copia va **sin fotos ni firmas**: son el 92% del peso, y sin ellas el historial de Carga pasa de 4,50 MB a 541 KB. Filtra por tamaño y no por nombre de campo, ordena por fecha antes de recortar, y arma el cartel de "estás viendo datos del…". Producción no lo usa: ya guarda sus muestreos enteros. |
| **render-historial.js** | Lo que hace que los historiales no maten al celular (Hallazgo 5). El problema no era la cantidad de filas: cada fila metía el **registro entero en base64 —con sus fotos adentro— nueve veces** en su propio HTML, una por cada `onclick`. Eran **54,2 MB**, con una fila sola de 12,34 MB. Ahora la fila lleva una clave de dos caracteres y el registro se queda en memoria: **78 KB, 708 veces menos**. Aporta además el paginado de a 50, el `DocumentFragment` y el `rebotar()` de 300 ms que usan los cinco buscadores. |
| **cola-sync.js** | El motor que vacía las colas offline de Control de Carga, Calidad y Ticketera (Hallazgo 4). Antes cada módulo tenía su propio bucle y los tres compartían el mismo bug: la recursión vivía solo en el `.then()`, así que **un registro que fallaba frenaba la cola entera para siempre**, y como el cursor arranca desde el principio, tapaba a todos los que venían atrás. Ahora un fallo no frena al resto. Distingue el rechazo del backend (suma intento, a los 3 va a revisión manual) de la falta de señal (no suma, reintenta siempre), hace reintentos con espera de 1/5/15 minutos y muestra la barra de aviso con la pantalla de revisión. Producción no lo usa: nunca tuvo el bug. |
...
| **03_auth.gs** | Login, contraseñas personales y recuperación por correo. El navegador deriva un verificador con PBKDF2 (150.000 vueltas) y acá se compara contra la hoja `Usuarios`: la contraseña nunca sale del dispositivo. Manda el código de recuperación de 6 dígitos con `MailApp`, con vencimiento, un solo uso y tope de intentos. Detalle en [DOCUMENTACION_CONTRASENAS.md](DOCUMENTACION_CONTRASENAS.md). |
...
|---|---|
| **AUDITORIA_2026-08-28.md** | **Revisión completa del proyecto buscando bugs y riesgos**, ordenada por urgencia y con la evidencia de cada hallazgo. Lo más grave: el backend no tiene autenticación y la URL está en un repo público; la contraseña está en un comentario; el Service Worker nunca se registra (la app no abre sin internet); una carga fallada traba toda la cola offline. Incluye también lo que se revisó y está bien, y por dónde empezar. |
| **MIGRACION_SUPABASE.md** | Evaluación de sacar la base del Google Sheet y llevarla a Supabase: los límites del plan gratuito contra los datos reales del proyecto, qué hallazgos de la auditoría cierra, el plan por fases y los **disparadores** para saber cuándo conviene hacerlo. Conclusión: entra cómodo en el plan gratis, pero todavía no hace falta. |
...
| **DOCUMENTACION_CONTRASENAS.md** | **Contraseñas personales y recuperación por correo.** Cómo se valida sin que la contraseña salga del dispositivo, por qué la validación tuvo que mudarse al servidor, cómo se entra sin señal, los frenos del código de recuperación y qué hacer si alguien queda afuera. |
