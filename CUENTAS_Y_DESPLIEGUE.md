...
| El chequeo de "ya existe este Id_Carga" **no era atómico** | Apps Script atiende varios POST a la vez: dos pedidos de la misma carga leían "no existe" antes de que cualquiera escribiera e insertaban los dos. La carga `BC-1787247191230` quedó con **3 filas en `Orden`, 9 productos y 3 contratos** siendo una sola carga real. | El chequeo y el `appendRow` van adentro de un `LockService` + `SpreadsheetApp.flush()`. Para limpiar lo viejo: `diagnosticoDuplicadosCarga()` y `limpiarDuplicadosCarga()` de [99_diagnostico.gs](99_diagnostico.gs). |
